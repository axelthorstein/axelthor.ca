...
def concat_species_genes(filenames, dir_name, file_type, filename):
	...
	# Format names
	if file_type == 'fas':
		formatted_names = format_names("fas", names)
	elif file_type == 'nex':
		...
		gene_length,
		filename)
		formatted_names = format_names("nex", names)
	elif file_type == 'phy':
		header = create_header(" {0} {1}\n", num_species, gene_length)
		formatted_names = format_names("phy", names)
...
def write_genes(names, file_type, filename, header):
	...
	directory = './gene_sequence_files/'

	output_file = open_file(file_type, directory + filename)
...
if __name__ == '__main__':
	...
	parser.add_argument('--type', choices=["fas", "nex", "phy"], 
		help='Sepcify output file type [fas, nex, phy]')
